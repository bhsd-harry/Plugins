      ...
      if (
        ...
        var closeBackdrop = function () {
          ...
          if (modalObj.options.keepContent !== true) {
            $backdrop.remove() //remove backdrop if keepContent option is false
          }
        }
    ...
    var closeModal = function () {
      ...
      if (modalObj.options.stack) {
        $modal.addClass('ssi-smoothSlide').slideUp('500', function () {
          ...
          if (modalObj.options.keepContent !== true) {
            $modal.remove() //will remove modal from DOM if keepContent option is false
          }
        })
      } else {
        $modal.trigger('onClose.ssi-modal') //trigger close event
        if (modalObj.options.keepContent !== true) {
          $modal.remove() //will remove modal from DOM if keepContent option is false$modal.remove()
        }
      }
